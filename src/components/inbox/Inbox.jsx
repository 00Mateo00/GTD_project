...
export const Inbox = () => {
	...
	useEffect(() => {
		const events = filteredTicklerFileEvents
			.filter((e) => dayjs(e.day).format("DD-MM-YY") === dayjs().format("DD-MM-YY"))
			.sort((a, b) => b.id - a.id)
	...
	return (
		<div className="inbox-wrapper">
			<div className="inbox-Tickler" ref={ref} onClick={() => setEventType("Tickler")}>
				{TicklerEvents}
			</div>
			<div className="grid-inbox">
				<div className="calendar-dayView" onClick={() => setEventType("Calendar")}>
					<DayView />
				</div>
				<div className="grid-container" onClick={() => setEventType("Actionables")}>
					{ActionableEvents}
				</div>
			</div>
			{onShowModal && eventType !== null ? (
				<MenuModal
					selected={
						(eventType === "Actionables" && selectedActionableTODO) || (eventType === "Tickler" && selectedTicklerFileEvent) || (eventType === "Calendar" && selectedCalendarEvent)
					}
					setSelected={
