...
export const DayView = () => {
  ...
  useEffect(() => {
    const events = filteredCalendarEvents.filter((e) => dayjs(e.day).format("DD-MM-YY") === daySelected.format("DD-MM-YY"));
    setDayEvents(events);
  ...
  return (
    <>
      {onShowModal && <MenuModal selected={selectedCalendarEvent} setSelected={setSelectedCalendarEvent} dispatchCall={dispatchCallCalendarEvent} />}
      <div className="times-wrapper">
        <div className="aside-hours-wrapper">
          <div className="aside-hours hours-grid">{hoursArray.map((e, i) => i < 23 && <div key={i}>{(i < 9 ? "0" : "") + (i + 1) + ":00"}</div>)}</div>
        </div>
        <div className="time-table hours-grid">
          {hoursArray.map((e, i) => (
            <div
              onClick={() => {
                if (window.location.pathname === "/GTD_project/Inbox") return;
                setHourClicked(i);
